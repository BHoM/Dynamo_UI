...
namespace Robot
{
    ...
    public class RobotNode
    {
        ...
        public static Dictionary<string, object> ByNodes(IEnumerable<BHoM.Structural.Node> nodes, bool allowRenumbering = true, bool activate = false)
        {
        ...
        public static Dictionary<string, object> Import(bool activate = false, bool allNodeData = false)
            ...
            if (activate == true)
            {
                ...
                RobotToolkit.Node.GetNodesQuery(out str_nodes);
                foreach (BHoM.Structural.Node str_node in str_nodes.Values)
                {
                    Point pnt = Point.ByCoordinates(str_node.X, str_node.Y, str_node.Z);
                    try { points.Add(str_node.Number, pnt); } catch { }
                }
                getnodes_out.Add("Points", points.Values);
                getnodes_out.Add("Numbers", nod_nums.ToList());
                getnodes_out.Add("Nodes", str_nodes.Values);
              }
            return getnodes_out;
            }
