...
namespace Structural.FormFinding
{
    ...
    public class DynamicRelaxation : IGraphicItem
    {
        ...
        {
            List<BHE.Bar> BHoMBars = new List<BHE.Bar>();
            List<BHE.Node> BHoMNodes = new List<BHE.Node>();
            ...
            /*if (run)   // TODO - Code does not compile -- please fix
            {
                foreach (Line ln in lines)
                    BHoMBars.Add(new BHE.Bar(new BHoM.Geometry.Point(ln.StartPoint.X, ln.StartPoint.Y, ln.StartPoint.Z), new BHoM.Geometry.Point(ln.EndPoint.X, ln.EndPoint.Y, ln.EndPoint.Z)));

                foreach (Point pt in lockedPoints)
                    BHoMNodes.Add(new BHE.Node(pt.X, pt.Y, pt.Z));
